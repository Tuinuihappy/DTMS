...
//
// Signed storage URLs are the wrong thing to put in an <img> on a list page: the
// signature carries a timestamp, so every list call mints a different URL, the
// browser treats each as a new file, and a table of photos re-downloads itself
// every time someone touches a filter. An attachment's image never changes — a
// new upload gets a new id — so the id is a perfectly good cache key. This route
...
export async function GET(
  ...
  try {
    ...
    if (api.status !== 302) {
      ...
      return fail(passOn ? api.status : 502, "Thumbnail unavailable.");
    }
    ...
    if (object.status !== 200 || !type.startsWith("image/")) {
      ...
      return fail(object.status === 404 ? 404 : 502, "Thumbnail unavailable.");
    }
    ...
    console.error(`[thumbnail] ${owner}/${ownerId}/${id} ${timedOut ? "TIMEOUT" : "ERROR"}:`, err);
    return fail(
      timedOut ? 504 : 502,
