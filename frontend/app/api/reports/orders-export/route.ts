...
// CSV export — bypasses the JSON proxy helper because we want to forward
// the raw stream (no .text() parse) and preserve Content-Disposition so
...
export async function GET(req: NextRequest) {
  ...
  const url = `${base.replace(/\/$/, "")}/api/v1/reports/orders-export${qs ? `?${qs}` : ""}`;
