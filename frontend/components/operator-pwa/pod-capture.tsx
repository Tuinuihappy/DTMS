...
export function PodCapture({ tripId, kind, podKey, onCaptured }: Props) {
  ...
  // Two phases, not one "busy". Resizing a 12 MP camera photo happens before
  // any request exists, so labelling it "Uploading" tells an operator watching
  // a still screen that the network is slow when the network has not been
  // touched yet.
