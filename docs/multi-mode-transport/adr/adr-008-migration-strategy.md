...
- ใช้ existing migrations เป็น template: [src/Modules/Dispatch/DTMS.Dispatch.Infrastructure/Migrations/](../../../src/Modules/Dispatch/DTMS.Dispatch.Infrastructure/Migrations/)
...
dotnet run --project src/DTMS.Api  # auto-applies all migrations
```
...
- name: Migration smoke test
  run: |
    ...
    timeout 60 dotnet run --project src/DTMS.Api --no-build &
    APP_PID=$!
    sleep 30
    kill $APP_PID || true
...
- Existing migrations: [src/Modules/Dispatch/.../Migrations/](../../../src/Modules/Dispatch/DTMS.Dispatch.Infrastructure/Migrations/)
