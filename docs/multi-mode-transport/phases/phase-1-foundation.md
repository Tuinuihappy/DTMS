...
git mv src/Modules/VendorAdapter/DTMS.VendorAdapter.Abstractions \
       src/Modules/Transport.Abstractions/DTMS.Transport.Abstractions

git mv src/Modules/VendorAdapter/DTMS.VendorAdapter.Riot3 \
       src/Modules/Transport.Amr/DTMS.Transport.Amr

git mv src/Modules/VendorAdapter/DTMS.VendorAdapter.Feeder \
       src/Modules/Transport.Amr/DTMS.Transport.Amr.Feeder

git mv src/Modules/VendorAdapter/DTMS.VendorAdapter.Simulator \
       src/Modules/Transport.Amr/DTMS.Transport.Amr.Simulator

git mv src/Modules/VendorAdapter/DTMS.VendorAdapter.Infrastructure \
       src/Modules/Transport.Amr/DTMS.Transport.Amr.Infrastructure
```

### Step 2: Update .csproj files (rename inside each .csproj)

ในแต่ละ .csproj ปรับ:
- `<AssemblyName>` → `DTMS.Transport.Amr.*`
- `<RootNamespace>` → `DTMS.Transport.Amr.*`
- `<ProjectReference>` ที่ชี้ไปยัง `VendorAdapter.Abstractions` → ชี้ไป `Transport.Abstractions`

**Pattern (representative):**
- [src/Modules/Transport.Abstractions/DTMS.Transport.Abstractions/DTMS.Transport.Abstractions.csproj](../../../src/Modules/Transport.Abstractions/)
- [src/Modules/Transport.Amr/DTMS.Transport.Amr/DTMS.Transport.Amr.csproj](../../../src/Modules/Transport.Amr/)

### Step 3: Update Solution File

แก้ `DTMS.slnx` — replace ทุก path/name ที่ contain `VendorAdapter` → `Transport.Amr` หรือ `Transport.Abstractions`

### Step 4: Bulk Namespace Update

ใช้ IDE (Rider/Visual Studio) **Find and Replace in Solution**:
- `DTMS.VendorAdapter.Abstractions` → `DTMS.Transport.Abstractions`
- `DTMS.VendorAdapter.Riot3` → `DTMS.Transport.Amr`
- `DTMS.VendorAdapter.Feeder` → `DTMS.Transport.Amr.Feeder`
- `DTMS.VendorAdapter.Simulator` → `DTMS.Transport.Amr.Simulator`
- `DTMS.VendorAdapter.Infrastructure` → `DTMS.Transport.Amr.Infrastructure`

**Files affected: ~50** — most are `using` statements. Some are inline references (e.g. `nameof()`).

### Step 5: Add New Abstractions

Create new files in `src/Modules/Transport.Abstractions/DTMS.Transport.Abstractions/Services/`:
...
**Create** `src/Modules/Transport.Amr/DTMS.Transport.Amr/TransportAmrServiceCollectionExtensions.cs`:

```csharp
namespace DTMS.Transport.Amr;
...
`src/Modules/Transport.Manual/DTMS.Transport.Manual/TransportManualServiceCollectionExtensions.cs`:
```csharp
...
**[ModuleServiceRegistration.cs](../../../src/DTMS.Api/Modules/ModuleServiceRegistration.cs)** ลบ inline registrations ที่ line 296-299, แทนด้วย:
...
**`src/Modules/Transport.Amr/DTMS.Transport.Amr/Application/Services/AmrDispatchStrategy.cs`**:
...
**`src/Modules/Transport.Amr/DTMS.Transport.Amr/Application/Services/VendorOperationsRouter.cs`**:
...
dotnet test tests/Integration/DTMS.IntegrationTests/ --no-build
...
```
1. Start API: dotnet run --project src/DTMS.Api
...
4. Trigger dispatch (via planning) → verify outbound RIOT3 call
   (use Simulator: src/Modules/Transport.Amr/DTMS.Transport.Amr.Simulator)
...
public void DispatchModule_ShouldNotReferenceTransportAmr()
{
    ...
    var amrAssemblyName = "DTMS.Transport.Amr";
