...
- ~~OpenAPI metadata for `Idempotency-Key` header~~ ✅ done — `IdempotencyKeyOperationTransformer` registered in [Program.cs:31](src/AMR.DeliveryPlanning.Api/Program.cs#L31) reads `IdempotencyKeyRequiredMetadata` marker and injects the header parameter into every endpoint that calls `.RequireIdempotencyKey()`
- **Cross-module reuse** — filter currently lives in `DeliveryOrder.Presentation/Idempotency/`; only 12 DeliveryOrder endpoints use `.RequireIdempotencyKey()`. To extend to Planning/Dispatch/Fleet/Facility mutation endpoints, move the `Idempotency/` folder to `SharedKernel` (or a new `AMR.DeliveryPlanning.Web` project), update DI registration, then tag the relevant endpoints. **Effort:** M; **Trigger:** duplicate-write incident in another module
