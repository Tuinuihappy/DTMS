...
| P4.F7 | Live list updates via OrderHub | ✅ Done (2026-06-16) — `useOrderListSubscription` in `lib/realtime/hubs/order-hub.ts`, `orders-experience.tsx` subscribes + 500 ms debounce-refetches `fetchOrders({silent:true})`. Hint payload `{orderId, toStatus}` is opaque — refetch keeps server FTS/facets/sort authoritative. |
