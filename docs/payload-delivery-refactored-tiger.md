...
| **P1-1** ServiceWindow VO `{earliest, latest}` replaces `RequestedDeliveryDate`; entity, commands, query DTO, integration event (Deadline→Earliest+Latest), AmendmentType rename `RequestedTimeChange`→`ServiceWindowChange`, migration | ✅ done | _this commit_ |
...
|---|---|---|---|
| ~~P1-1~~ | ~~**`serviceWindow{earliest, latest}`** ที่ order-level (แทน `RequestedDeliveryDate` เดียว)~~ ✅ done (this commit) — VO with nullable bounds (≥1 required, Earliest≤Latest), replaces RequestedDeliveryDate; Confirmed event Deadline→Earliest+Latest (clean break — Planning consumer unaffected); AmendmentType `ServiceWindowChange` | M | §1 line 88 |
...
2. ~~**P1-2 (slaTier + submittedAt)**~~ ✅ done — enum + SLA clock + additive integration event
3. ~~**P1-6 (`:hold` / `:release` endpoints)**~~ ✅ done — expose entity methods via REST
4. ~~**P1-1 (serviceWindow)**~~ ✅ done — VO replacement; integration event Deadline→Earliest+Latest
