...
| **P0-2 reverted** — code-only (YAGNI: no upstream client submits via stationId) | ✅ done | _this commit_ |
...
| **P0-4 relaxed** — header changed to **optional / best-effort** (filter passes through when missing; replay/conflict logic intact when present); OpenAPI marks header `required: false` | ✅ done | _this commit_ |
...
| **Decisions 1–5** resolved (path-based versioning / keep `Confirmed` / no multi-tenant / UOM hybrid / Hazmat Option C) | ✅ done | _this commit_ |
| **P1-7** Path versioning — prefix `/api/v1/` ทุก business module (DeliveryOrder/Items/Fleet/Dispatch/Facility/Planning); Auth/Webhook/Health unchanged | ✅ done | _this commit_ |
...
| `POST /api/v1/orders/{id}:hold` | — | ❌ pending P1-6 (entity มี `Hold()` แต่ไม่มี endpoint) |
| `POST /api/v1/orders/{id}:release` | — | ❌ pending P1-6 |
...
| ~~P0-2 reverted~~ | ~~**Tagged union over-engineered**~~ — ไม่มี upstream client ส่ง stationId form จริง (SAP/ERP/Oms/Manual ใช้ code) | ลบ `LocationRef` VO + `LocationRefDto`/`LocationRefMapper`; เปลี่ยน `Item.PickupLocation/DropLocation` → `PickupLocationCode/DropLocationCode` (string); `StationValidationService` key เป็น string; new migration drop `PickupLocationStationId`/`DropLocationStationId` columns + ALTER code columns NOT NULL | YAGNI — เพิ่มกลับเมื่อมี client ที่ต้องการ Guid form จริง (additive change) | ✅ _this commit_ |
...
### Execution order ที่แนะนำ (low-coupling → high-coupling)

1. ~~**P1-7 (path versioning)**~~ ✅ done — prefix `/api/v1/` ทุก business module
2. **P1-2 (slaTier + submittedAt)** [S] — เพิ่ม field ที่ order-level, ไม่กระทบ Planning solver ทันที
3. **P1-6 (`:hold` / `:release` endpoints)** [S] — entity มี method แล้ว แค่ expose API
4. **P1-1 (serviceWindow)** [M] — replace `RequestedDeliveryDate` เดี่ยวด้วย `{earliest, latest}`; กระทบ Planning consumer event payload
5. **P1-9 (Quantity VO + UOM enum)** [M] — domain refactor + migration backfill (alias map config)
6. **P1-3 (HazmatInfo VO)** [M] — extend Item; coordinate กับ Planning segregation table (separate effort)
7. **P1-4 (TemperatureRange VO)** [S]
8. **P1-5 (handlingInstructions)** [S]
9. **P1-8 (event versioning + `schemaVersion`)** [M] — ทำหลัง P1-1/P1-3/P1-4 เพราะต้อง bump schema version ครั้งเดียว
10. **P1-10 (amendment snapshot scope)** [M] — ทำท้ายสุด หลัง field ใหม่นิ่ง
