...
| **P1-1** ServiceWindow VO `{earliest, latest}` replaces `RequestedDeliveryDate`; entity, commands, query DTO, integration event (Deadline→Earliest+Latest), AmendmentType rename `RequestedTimeChange`→`ServiceWindowChange`, migration | ✅ done | `d9c4e93` |
