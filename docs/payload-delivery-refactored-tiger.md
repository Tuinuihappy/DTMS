...
- OpenAPI metadata for `Idempotency-Key` header — Swagger UI ไม่แสดง required header (`Microsoft.AspNetCore.OpenApi` reference only in Api project, not Presentation). Add by registering filter at Api layer
- Cross-module reuse — filter currently lives in `DeliveryOrder.Presentation/Idempotency/`; ถ้าจะใช้กับ Planning/Dispatch/Fleet/Facility ต้องย้ายเป็น shared web project

**P0-5 — Weight policy**
- Refine `WeightFallback` — ปัจจุบัน flat 500 kg. ขยายเป็น per `LoadUnitProfile.MaxGrossWeightKg` (cross-module Facility lookup) หรือ fleet max (Fleet lookup)
- `GET /api/v1/delivery-orders/{id}` ยังไม่ surface warnings — ต้องเรียก `/timeline` แล้วกรอง `QualityWarning` events. พิจารณาเพิ่ม `qualityIssues[]` ใน detail response
- **Warning catalog versioning** — ถ้าเพิ่ม code ใหม่ (e.g. `ITEM_DIMS_MISSING`, `STATION_NEAR_CAPACITY`) ควรมี registry + docs

### Design-doc sync (cosmetic)

- Refresh `AMR_Delivery_Planning_System_Design.md §1` payload schema sample to reflect P1 shape (`serviceWindow`, `slaTier`, `submittedAt`, item-level `hazmat`/`temperature`/`handlingInstructions`, `Quantity{value, uom}`, `SchemaVersion`)
- Add semver convention block (major = class rename / V2 introduction; minor = additive field; `SchemaVersion = "X.Y"`)
- Document `OrderSnapshotV1` and `AmendmentVersion` column semantics
