...
| 5 | P1-9 Quantity VO + UOM enum | ⏳ next | — |
...
1. **P1-9** Quantity VO + UOM enum **[M]** — domain refactor (closed `UnitOfMeasure` enum KG/G/LB/EA/BOX/PALLET/CASE + alias map in `appsettings.json` per Decision #4); migration to convert existing `Uom` string → enum; backfill unknown values; ratchet upstream/draft/submit validators
