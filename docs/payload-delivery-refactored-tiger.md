...
| **P1-8** Event versioning — rename 8 integration events to `…V1` suffix; add `SchemaVersion = "1.0"` field; drop `Deadline` backward-compat alias (Planning consumer reads `Latest`); semver convention documented (major = class rename, minor = additive field) | ✅ done | _this commit_ |
...
| 9 | P1-8 event versioning + remove `Deadline` alias | ✅ | _this commit_ |
