...
| **P1-1** ServiceWindow VO `{earliest, latest}` replaces `RequestedDeliveryDate`; entity, commands, query DTO, integration event additive (`Deadline` retained as alias for `Latest`, slated for removal in P1-8), AmendmentType rename `RequestedTimeChange`→`ServiceWindowChange`, migration | ✅ done | `d9c4e93` + _this commit_ |
