...
## Integration test follow-ups (post `1415592`)

หลัง first pass cleanup — project compiles, 49/76 integration tests pass. เหลือ 16 fail แยกเป็น 4 หมวด:

| Category | Tests | Cause | Fix effort |
|---|---|---|---|
| **Un-built feature** — SLA timing validator | `SlaValidationTests` (4) | API ยังไม่ enforce "SLA ≥ 30min ahead" rule; `DeliveryOrderOptions.MinimumSlaLeadTimeMinutes` มีค่าแต่ไม่มี code อ่าน. Tests forward-written | Skip with `[Fact(Skip = "SLA timing validator not implemented")]` |
| **Response shape changed** — P0-5 wrapped result | `IdempotencyTests` (3), `AmendmentTimelineTests` (4) | API returns `{orderId, warnings}` ตอนนี้; test deserializes as `Guid` directly | Read wrapped result: `.ReadFromJsonAsync<{Id: Guid}>()` |
| **Enum casing** | `UpdateDraftOrderTests` (2) | API serializes enums uppercase (`"HIGH"`); test asserts `"High"` | Trivial — change assertion strings |
| **End-to-end flow** | `EndToEndFlowTests` (1), `EndToEndPipelineTests` (2) | Cascade — likely body/response shape from above | Investigate per test |

**Total remaining effort**: ~2-3 hours for full green. Recommend quick wins (casing, response-shape ratchet) first.
