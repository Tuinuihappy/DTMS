...
public static class DispatchEndpoints
{
    public static void MapDispatchEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // POST /api/v1/dispatch/trips — Dispatch a new trip from a committed job
        group.MapPost("/trips", async (DispatchTripCommand command, ISender sender) =>
        {
            var result = await sender.Send(command);
            return result.IsSuccess
                ? Results.Created($"/api/v1/dispatch/trips/{result.Value}", result.Value)
                : Results.BadRequest(result.Error);
        });

        // POST /api/v1/dispatch/trips/{id}/start — Start a created trip
        ...
        // POST /api/v1/dispatch/trips/{tripId}/tasks/{taskId}/complete — Report task completed
        ...
        // POST /api/v1/dispatch/trips/{tripId}/tasks/{taskId}/fail — Report task failed
        ...
        // POST /api/v1/dispatch/trips/{id}/reassign
