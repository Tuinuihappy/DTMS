...
/// operator + persists Trip with no vendor key.
///
...
public interface IDispatchStrategy
{
    ...
    /// by (PickupWarehouseId, DropWarehouseId). Returning N groups results
...
public sealed record DispatchGroupItem(
    ...
    Guid? DropStationId,
    // WMS PR-3 — the Manual strategy groups + dispatches by WMS location.
...
/// pair (station for AMR OR WMS location for Manual, never both
...
/// AMR uses every field; Manual ignores station Ids (it re-resolves via
/// the order's warehouse Ids) and the appoint-vehicle overrides
/// (Manual = operator-pick).
/// </summary>
public sealed record DispatchGroupRequest(
    Guid DeliveryOrderId,
    int GroupIndex,                       // 1-based
    Guid PickupStationId,                 // AMR uses; Manual ignores
    Guid DropStationId,
    ...
    // WMS PR-3 — Manual dispatch groups pass the WMS location Ids
