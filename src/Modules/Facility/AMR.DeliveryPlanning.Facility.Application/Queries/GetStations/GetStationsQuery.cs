...
public record GetStationsQuery(Guid? MapId, StationType? Type, Guid? ZoneId, string? CompatibleVehicleType, bool IncludeInactive = false) : IQuery<List<StationDto>>;
...
public class GetStationsQueryHandler : IQueryHandler<GetStationsQuery, List<StationDto>>
{
    ...
    {
        var stations = await _repo.QueryAsync(request.MapId, request.Type, request.ZoneId, request.CompatibleVehicleType, request.IncludeInactive, cancellationToken);
