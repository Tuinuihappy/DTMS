...
/// <summary>
/// Formats a <see cref="ShipmentPickedUpContext"/> into the body OMS's TMS
/// integration endpoint expects at
/// <c>POST /integrations/tms/shipments/{shipmentId}/pickup-arrived</c>:
/// <c>{ "orderRef": ..., "locationCode": ..., "occurredAt": ... }</c>
/// (shipmentId travels in the path, mirroring the arrived formatter).
/// This formatter IS the contract; the byte-compat tests pin it.
/// Resolved by keyed DI under <see cref="FormatKey"/>.
/// </summary>
public sealed class OmsShipmentPickedUpFormatter : ICallbackPayloadFormatter
{
    public const string FormatKey = "oms.shipment.pickedup.v1";

    // OMS's example pins millisecond precision ("...T07:28:59.989Z") — same
    // rationale as OmsShipmentStartedFormatter: a raw DateTime would
    // serialise 7 fractional digits, which a strict parser may reject.
    ...
    {
        if (integrationEvent is not ShipmentPickedUpContext ctx)
            throw new InvalidOperationException(
                $"{nameof(OmsShipmentPickedUpFormatter)} expects {nameof(ShipmentPickedUpContext)} " +
        ...
        var path = $"/integrations/tms/shipments/{Uri.EscapeDataString(ctx.ShipmentId)}/pickup-arrived";
